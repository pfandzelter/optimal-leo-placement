...
def prepare_calculation(name: str, planes: int, sats: int, d_intra: float, d_inter_mean: float, d_inter_max: float) -> typing.Tuple[typing.Dict[str, typing.List[int]], typing.List[int], typing.List[int]]:
    ...
    for t in config.SLO:
        print("Retrieving placement {}-{}".format(t["type"], t["t"]))
        t["p"] = pd.read_csv(os.path.join(config.PLACEMENTS_DIR, "{}_{}_{}.csv".format(name, t["type"], t["t"])))
    ...
    with open(os.path.join(results_folder, "{}-{}.csv".format(name, next_time)), "w") as f:
        ...
        for t in SLOs:
            for n in range(planes * sats):
                r = indices[rns["{}-{}".format(t["type"], t["t"])][n]]
                f.write("{},{},{},{},{}\n".format(t["type"], t["t"], n, rns["{}-{}".format(t["type"], t["t"])][n], dist_matrix[r, n]))



if __name__ == "__main__":
    for s in config.SHELLS:
        s["rns"], s["considered_rnodes"], s["indices"] = prepare_calculation(s["name"], s["planes"], s["sats"], s["d_intra"], s["d_inter_mean"], s["d_inter_max"])

    print("Going through simulation steps")

    # for e in [(s["name"], s["planes"], s["sats"], s["rns"], s["considered_rnodes"], s["indices"], t, config.SLOs, results_folder) for s in config.shells for t in range(0, SIM_LENGTH, config.INTERVAL)]:
    #     calculate(e)
    ...
    #     executor.map(calculate, [(s["name"], s["planes"], s["sats"], s["rns"], s["considered_rnodes"], s["indices"], t, config.SLOs, results_folder) for s in config.shells for t in range(0, SIM_LENGTH, config.INTERVAL)])
